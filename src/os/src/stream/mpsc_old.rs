...
pub struct MpscSender<T, S> where
    S: Subscriber<T>, S: 'static + Send, T: 'static + Send
{
    subscriber: Unique<SubscriberLink<T, S>>
}

impl<T, S> MpscSender<T, S> where
    S: Subscriber<T>, S: 'static + Send, T: 'static + Send
{
    pub fn new(subscriber: S) -> MpscSender<T, S> {
        unsafe{ MpscSender {
            subscriber: Unique::new(into_raw(Box::new(
                SubscriberLink {
                    queue: Queue::new(),   // unlimited cache
...
impl<T, f> MpscSender<T, FnSubscriber<f>> where
    T: 'static + Send, f: FnMut(T) + Send
{
    pub fn on_value(on_value: f) -> MpscSender<T, FnSubscriber<f>> {
        Self::new(FnSubscriber(on_value))
    }
}

impl<T, S> Drop for MpscSender<T, S> where
    ...
    S: Subscriber<T> + 'static + Send, T: 'static + Send
{
    ...
    unsafe fn pop_values(&mut self) {
        while let PopResult::Data(value) = self.queue.pop() {
